...
function taskForProjectBoard(t: any, extras: any = {}) {
  ...
  return {
    ...
    qaSignoffUserId: t.qaSignoffUserId ? String(t.qaSignoffUserId) : undefined,
    qaSignoffAt: toIso(t.qaSignoffAt),
  ...
  try {
    ...
    const [team, owner, tasks] = await Promise.all([
      ...
      Task.find({ projectId: p._id })
        ...
        .select('projectId phaseId title description assigneeId status priority taskType gxpCritical requiresQaSignoff qaSignoffUserId qaSignoffAt startDate dueDate completedAt ccNo ccTcd documentNo applicableSite deployStage remarks pendingWith position createdAt updatedAt subtasks.status')
