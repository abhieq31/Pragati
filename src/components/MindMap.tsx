...
export function MindMap() {
  ...
  function addNode(at?: { x: number; y: number }) {
    ...
    setNodes((n) => [
      ...n,
      { id, x: pos.x, y: pos.y, text: 'New thought', color: COLORS[n.length % COLORS.length] },
    ]);
    setEditing(id);
  ...
  function onMouseUp(e: React.MouseEvent) {
    if (linking && linkCursor) {
      ...
      if (
        target &&
        !edges.some(
          (e) => (e.from === linking && e.to === target.id) || (e.from === target.id && e.to === linking),
        )
      ) {
  ...
  function commitEdit() {
    if (!editing) return;
    setNodes((arr) =>
      arr.map((n) => (n.id === editing ? { ...n, text: pendingText.trim() || 'Untitled' } : n)),
    );
    setEditing(null);
    setPendingText('');
    mark();
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-400 dark:text-white/30">
              Tap canvas to add · Drag to move · Link to connect
            </div>
          </div>
        </div>
          ...
          {nodes.map((n) => {
            ...
            return (
              <g
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (e.shiftKey) deleteNode(n.id);
                }}
                onDoubleClick={(e) => {
                  e.stopPropagation();
                  setEditing(n.id);
                  setPendingText(n.text);
                }}
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center max-w-sm">
              ...
              <div className="text-xs text-slate-400 mt-1">
                Tap anywhere on the canvas to drop a node. Drag it to move, double-click to rename, drag the
                side handle onto another node to connect them.
              </div>
            </div>
          </div>
        )}
