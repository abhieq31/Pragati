...
import {
  LayoutDashboard, FolderKanban, Users, UsersRound, NotebookPen, ChevronLeft,
    ...
    <>
      ...
      <div className="px-3 py-3 border-t shrink-0 relative"
        ...
        <div className={`flex items-center gap-2 rounded-lg px-1.5 py-1.5 ${dark ? '' : ''}`}>
          ...
          {/* Notifications + signout collapse into a mini row above avatar when collapsed */}
          {collapsed ? (
    ...
    <div className="min-h-screen flex" style={{ background: 'var(--bg-page)' }}>
      ...
      >
        <button
          type="button"
          onClick={() => setCollapsed((c) => !c)}
          className={`hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 z-20 w-6 h-6 rounded-full items-center justify-center border ${
            dark ? 'bg-[#30302e] border-white/15 text-white/60 hover:text-white' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-700'
          }`}
          title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
          aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
        >
