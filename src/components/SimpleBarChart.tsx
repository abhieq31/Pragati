...
export function SimpleBarChart({ data, series, height = 280, legend = true }: SimpleBarChartProps) {
  ...
  const barGap = 4;
  const barWidth =
    data.length > 0 ? Math.max(4, (groupWidth - barGap * (series.length + 1)) / series.length) : 0;
  ...
  return (
    <div ref={wrapRef} className="relative w-full" style={{ height }}>
      ...
      {hover !== null && data[hover.row] && (
        ...
        >
          <div className="font-semibold text-slate-700 mb-0.5">{String(data[hover.row].label ?? '')}</div>
          {series.map((s) => (
