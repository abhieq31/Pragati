...
export default function Dashboard() {
  ...
  // Ongoing projects only — leads care about active work, not archives.
  const ongoingProjects = useMemo(() => {
    if (!dash) return [];
    return dash.projects.filter(p =>
      p.status === 'in_progress' || p.status === 'planning' || p.status === 'on_hold',
    );
  }, [dash]);

  // Group tasks by project for the Projects column
  const tasksByProject = useMemo(() => {
    if (!dash) return new Map<string, TeamTask[]>();
  ...
  // Group tasks by assignee for the Contributors column
  ...
  return (
    <div className="pb-12 max-w-[1440px]">
      ...
      <div className="flex gap-2.5 mb-6">
        {[1,2,3,4].map(i => <div key={i} className="skeleton h-9 w-28 rounded-lg" />)}
      </div>
