...
export default function TeamDetailClient({
  ...
  // Seed from server-rendered initial data — no post-hydration waterfall.
  const [team, setTeam] = useState<any>(initialTeam ?? null);
  const [board, setBoard] = useState<any[]>(initialBoard ?? []);
  // Progress analytics are lead-only and fetched client-side (secondary data).
  ...
  // Post-mutation refresh — re-fetches both team + board from the API.
  // NOT called on initial mount (server already seeded the data).
  async function load() {
    setLoadError('');
    try {
  ...
  useEffect(() => {
    // Only fetch progress here (secondary, lead-only). Main data was SSR'd.
    if (isLead && id) {
