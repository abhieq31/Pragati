...
export async function POST(req: NextRequest) {
  try {
    ...
    const project = await Project.findOne({ _id: body.projectId, ...projectsVisibleFilter(scope) }).select('_id ownerId isPersonal').lean();
    ...
    if (!canMutate(user!.role) && !personalOwner) {
      return NextResponse.json({ error: 'Insufficient permissions to create tasks here' }, { status: 403 });
    }
