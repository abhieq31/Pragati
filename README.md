...
- **Daily support-ticket tracking** — opt-in per project (for teams like Quality Informatics who report a daily ticket count upward): log open / new / resolved each morning and the app computes the backlog trend, 7-day inflow-vs-throughput net flow, a robust anomaly flag, and a "clears in ~N days" projection. The count shows on the project, in every report (Excel gets a dedicated sheet), and rolled up in the lead/admin Morning Brief and 08:30 email — all deterministic, no LLM.
