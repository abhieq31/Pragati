...
- **A living login screen** — unattributed lines from *The Almanack of Naval Ravikant* and the books Naval recommends (Munger, Marcus Aurelius, Seneca, Bruce Lee, Taleb, Feynman, …) on leverage, judgment, and compounding, never repeating on a device until the whole library has cycled, refreshable forever via `QUOTES_FEED_URL` (a public JSON you host) with the built-in library as permanent fallback.
