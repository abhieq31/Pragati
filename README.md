...
- **A living login screen** — unattributed lines from *The Almanack of Naval Ravikant* on leverage, judgment, and compounding, never repeating until the whole library has cycled, daily-seeded, refreshable forever via `QUOTES_FEED_URL` (a public JSON you host) with the built-in library as permanent fallback.
